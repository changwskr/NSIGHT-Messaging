...
(function () {
    ...
    const API = '/api/v1/trace-environment';
    ...
    function renderCheckReportStackConfigHtml(view) {
        ...
        if (!grid.length && !layers.length) {
            return `<p class="env-check-report__placeholder">
                계층 설정 없음 — <a href="/traceenvironment/env-002">ENV-002</a> 산정 후 새로고침하세요.
            </p>`;
        }
    ...
    function renderCheckIntegratedReport() {
        ...
        if (!view?.planner) {
            ...
            panel.innerHTML = `
                <p class="env-check-report__placeholder">
                    ...
                    <a href="/traceenvironment/env-002">ENV-002</a>에서 조건 입력 후 「산정 실행」을 하고 이 페이지를 새로고침하세요.
                </p>
                <p class="env-check-report__placeholder">
                    Rule Engine: <a href="/traceenvironment/rule-check">Rule 점검</a>에서 설정 업로드·「점검 실행」 후 이 보고서에 Rule 결론이 추가됩니다.
                </p>`;
            return;
        }
        ...
        const ruleBlock = assessment
            ...
            : `<div class="env-check-report__rule env-check-report__rule--pending">
                <h4>6. Rule Engine</h4>
                <p>미실행 — <a href="/traceenvironment/rule-check">Rule 점검</a>에서 「설정 업로드·파싱」 후 「점검 실행」하면 이 보고서에 Rule 결론이 반영됩니다.</p>
               </div>`;

        const ruleDetailLink = assessment
            ? `<p class="env-check-report__link"><a href="/traceenvironment/rule-check">Rule 점검 상세 →</a></p>`
            : '';
        ...
        panel.innerHTML = `
            ...
            <div class="env-check-report__grid">
                <section class="env-check-report__sect">
                    ...
                    <p class="env-check-report__link"><a href="/traceenvironment/env-002">ENV-002 상세 →</a></p>
                </section>
                <section class="env-check-report__sect">
                    ...
                    <p class="env-check-report__link"><a href="/traceenvironment/env-003">ENV-003 전체 표 →</a></p>
                </section>
                <section class="env-check-report__sect env-check-report__sect--env004">
                    ...
                    <p class="env-check-report__link">상세 구성은 아래 <strong>§5 환경 구성</strong> · <a href="/traceenvironment/env-004">ENV-004</a></p>
                </section>
            ...
            <section class="env-check-report__sect env-check-report__sect--stack">
                ...
                <p class="env-check-report__link"><a href="/traceenvironment/env-004">ENV-004 Grid · 파이프라인 →</a></p>
            </section>
            ${ruleBlock}
            ${ruleDetailLink}
            <p class="env-check-report__footer">보고서 생성: 브라우저 산정 캐시 · Rule은 <a href="/traceenvironment/rule-check">Rule 점검</a> 실행 시 갱신</p>`;
    }
