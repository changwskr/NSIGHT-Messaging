...
public final class SpRouteKeyResolver {
    ...
    private static final Map<String, String> LEGACY_TX_CODE = Map.of(
            "71001", "7001",
            "71002", "7101",
            "72001", "7201");
    ...
    public static String transactionCode(EPlatonEvent event, String serviceId) {
        ...
        if (eventNo.startsWith(serviceId)) {
            return eventNo.substring(serviceId.length());
        }
        String legacyServiceId = LEGACY_EVENT_NO.get(eventNo);
        if (serviceId.equals(legacyServiceId)) {
            return LEGACY_TX_CODE.getOrDefault(eventNo, "");
        }
        return "";
    }
